...
export default function Templates() {
  ...
  const [formData, setFormData] = useState({ name: "", content: "", pdfUrl: "", pdfName: "", type: "whatsapp", subject: "" });
  ...
  const openEdit = (template: any) => {
    ...
    setFormData({ name: template.name, content: template.content, pdfUrl: template.pdfUrl || "", pdfName: template.pdfName || "", type: template.type || "whatsapp", subject: template.subject || "" });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: "", content: "", pdfUrl: "", pdfName: "", type: "whatsapp", subject: "" });
      setPdfFile(null);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isOpen} onOpenChange={(o) => { setIsOpen(o); if (!o) { setEditingTemplate(null); setFormData({ name: "", content: "", pdfUrl: "", pdfName: "", type: "whatsapp", subject: "" }); setPdfFile(null); } }}>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Template Type</Label>
                  <div className="flex gap-2">
                    {[
                      { value: "whatsapp", label: "WhatsApp / SMS", color: "text-green-400 border-green-400/30 bg-green-400/10" },
                      { value: "email",    label: "Email",           color: "text-purple-400 border-purple-400/30 bg-purple-400/10" },
                      { value: "both",     label: "Both",            color: "text-primary border-primary/30 bg-primary/10" },
                    ].map(opt => (
                      <button key={opt.value} type="button"
                        onClick={() => setFormData({ ...formData, type: opt.value })}
                        className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-all ${
                          formData.type === opt.value ? opt.color : "border-white/10 bg-white/5 text-muted-foreground hover:border-white/20"
                        }`}>
                        {opt.label}
                      </button>
                    ))}
                ...
                {/* Email subject — only for email/both types */}
                {(formData.type === "email" || formData.type === "both") && (
                  <div className="space-y-2">
                    <Label htmlFor="subject" className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Email Subject</Label>
                    <Input
                      id="subject"
                ...
                <div className="space-y-2">
                  <Label htmlFor="content" className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                    {formData.type === "email" || formData.type === "both" ? "Email Body" : "Message Content"}
                  </Label>
                  <Textarea
                    id="content"
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${
                      template.type === 'email' ? 'bg-purple-400/10 text-purple-400 border-purple-400/20'
                      : template.type === 'both' ? 'bg-primary/10 text-primary border-primary/20'
                      : 'bg-green-400/10 text-green-400 border-green-400/20'
                    }`}>
                      {template.type === 'email' ? '✉ Email' : template.type === 'both' ? '✉ + 💬 Both' : '💬 WhatsApp/SMS'}
                    </span>
                  </div>
                </div>
